...
export default function InvoiceEditor({ invoiceId }: { invoiceId?: string }) {
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      const paidEditConfirmationRequired = invoice?.payment_status === "paid" || paymentStatus === "paid";
      const confirmPaid = !paidEditConfirmationRequired || window.confirm("Invoice LUNAS sedang diedit. Pastikan perubahan data dan total sudah benar. Lanjutkan?");
      if (!confirmPaid) { setSaving(false); return; }
      const result = invoiceId
        ? await supabase.rpc("update_invoice", { p_invoice_id: invoiceId, ...args, p_status: invoiceStatus, p_payment_status: paymentStatus, p_payment_method: paymentMethod || null, p_confirm_paid: confirmPaid })
      ...
      window.setTimeout(() => window.location.assign("/admin/invoices/"), 500);
  ...
  return <>
    ...
    <form className="adminForm invoiceForm" onSubmit={save}>
      ...
      <section><h2>Status invoice</h2><div className="adminFormGrid"><label>Status invoice<select value={invoiceStatus} onChange={(event) => setInvoiceStatus(event.target.value as Invoice["status"])}><option value="draft">Draft</option><option value="issued">Terbit</option><option value="cancelled">Batal</option></select></label><label>Status pembayaran<select value={paymentStatus} onChange={(event) => setPaymentStatus(event.target.value as Invoice["payment_status"])}><option value="unpaid">Belum Dibayar</option><option value="paid">Lunas</option></select></label><label>Metode pembayaran<input value={paymentMethod} onChange={(event) => setPaymentMethod(event.target.value)} /></label></div></section>
      ...
      <button className="adminPrimary adminSubmit" disabled={saving} type="submit">{saving ? "Menyimpan…" : invoice ? "Simpan Perubahan" : "Simpan Draft"}</button>
    </form>
