...
type FormState = {
  name:string;sku:string;slug:string;category:string;categoryId:string;price:string;originalPrice:string;
...
const emptyForm: FormState = { name:"",sku:"",slug:"",category:"",categoryId:"",price:"",originalPrice:"",shortDescription:"",description:"",seoDescription:"",colors:"",occasions:"",tags:"",available:true,preorder:false,featured:false,bestseller:false,leadTime:"",sortOrder:"0",isActive:true };
...
function productPayload(form: FormState, categories: CategoryRow[], isActive = form.isActive) {
  const selectedCategory = categories.find((item) => item.id === form.categoryId);
  return {
    sku:form.sku.trim(),slug:form.slug.trim(),name:form.name.trim(),category:selectedCategory?.name ?? form.category,
...
function restoreProductPayload(product: ProductRow) {
  return {
    sku:product.sku,slug:product.slug,name:product.name,category:product.category,price:product.price,
...
export default function ProductEditor({ productId }: { productId?: string }) {
  ...
  useEffect(()=>{
    requireAdminSession().then(async({supabase})=>{
      const categoryResult=await supabase.from("categories").select("*").eq("is_active", true).order("sort_order", { ascending: true }).order("name", { ascending: true });
      if(categoryResult.error)throw categoryResult.error;
      const activeCategories = (categoryResult.data ?? []) as CategoryRow[];
      setCategories(activeCategories);

      const nextDefaultId = activeCategories[0]?.id ?? "";
      setForm(current => ({
        ...current,
        category: current.category || activeCategories[0]?.name || "",
      ...
      const selectedCategory = activeCategories.find((category) => category.id === p.category_id) ?? activeCategories.find((category) => category.name === p.category) ?? activeCategories[0];
      originalProduct.current=p;
      originalImages.current=rows;
      setForm({name:p.name,sku:p.sku,slug:p.slug,category:p.category,categoryId:selectedCategory?.id ?? "",price:String(p.price),originalPrice:p.original_price===null?"":String(p.original_price),shortDescription:p.short_description,description:p.description,seoDescription:p.seo_description,colors:p.colors.join(", "),occasions:p.occasions.join(", "),tags:p.tags.join(", "),available:p.available,preorder:p.preorder,featured:p.featured,bestseller:p.bestseller,leadTime:p.lead_time??"",sortOrder:String(p.sort_order),isActive:p.is_active});
  ...
  async function submit(event:FormEvent){
    ...
    if(!categories.length){setError("Kategori aktif belum tersedia. Tambahkan kategori terlebih dahulu.");return;}
    if(!form.categoryId){setError("Pilih kategori produk yang valid.");return;}
    ...
    try{
      ...
      const payload=productPayload(form, categories);
      let id=productId;
      if(!id){
