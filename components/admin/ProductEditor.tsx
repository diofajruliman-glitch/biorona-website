...
function productPayload(form: FormState, isActive = form.isActive) {
  return {
    sku:form.sku.trim(),slug:form.slug.trim(),name:form.name.trim(),category:form.category,
...
export default function ProductEditor({ productId }: { productId?: string }) {
  ...
  useEffect(()=>{
    ...
    requireAdminSession().then(async({supabase})=>{
      ...
      setForm({name:p.name,sku:p.sku,slug:p.slug,category:p.category,price:String(p.price),originalPrice:p.original_price===null?"":String(p.original_price),shortDescription:p.short_description,description:p.description,seoDescription:p.seo_description,colors:p.colors.join(", "),occasions:p.occasions.join(", "),tags:p.tags.join(", "),available:p.available,preorder:p.preorder,featured:p.featured,bestseller:p.bestseller,leadTime:p.lead_time??"",sortOrder:String(p.sort_order),isActive:p.is_active});
    ...
    try{
      ...
      if(supabase){
        ...
        if(uploadedPaths.length){
          let cleanup=await supabase.storage.from("product-images").remove(uploadedPaths);
          if(cleanup.error)cleanup=await supabase.storage.from("product-images").remove(uploadedPaths);
          if(cleanup.error)cleanupErrors.push("file upload baru perlu dibersihkan manual");
        }
