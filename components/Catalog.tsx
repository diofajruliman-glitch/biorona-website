...
export default function Catalog({ products, unavailableMessage, categories = [], variant = "catalog" }: Props) {
  ...
  if (variant === "home") return <section className="section catalogSection homeCatalog" id="katalog" aria-labelledby="featured-products-title"><div className="container"><div className="sectionHeading splitHeading"><div><span className="kicker">Kategori utama</span><h2 id="featured-products-title">Pilihan bunga untuk momen istimewa.</h2></div><p>Temukan buket bunga, standing flower, bunga ucapan, dan custom bouquet pilihan dari florist Biorona di Cibinong, Bogor.</p></div><div className="homeCategories" aria-label="Kategori bunga">{categoryOptions.filter((item) => item !== "Semua").slice(0, 6).map((item) => <a key={item} href={`/katalog?category=${encodeURIComponent(item)}`}>{item}</a>)}</div>{unavailableMessage ? <div className="emptyState" role="alert"><strong>{unavailableMessage}</strong><p>Anda tetap dapat menghubungi Biorona melalui WhatsApp untuk bantuan.</p></div> : <><div className="productGrid">{featured.map((product) => <ProductCard key={product.slug} product={product}/>)}</div><div className="catalogAllCta"><a className="secondaryGlassButton glassSurface" href="/katalog">Lihat Semua Koleksi <span aria-hidden="true">→</span></a></div></>}</div></section>;
