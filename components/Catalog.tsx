...
export default function Catalog({ products, unavailableMessage, categories = [], variant = "catalog" }: Props) {
  ...
  const featured = useMemo(() => { const selected = products.filter((product) => product.featured || product.bestseller); return (selected.length ? selected : products).slice(0, PAGE_SIZE); }, [products]);
  const totalPages = Math.max(1, Math.ceil(filtered.length / PAGE_SIZE));
  const currentPage = Math.min(requestedPage, totalPages);
  const visibleProducts = filtered.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE);

  if (variant === "home") return <section className="section catalogSection homeCatalog" id="katalog" aria-labelledby="featured-products-title"><div className="container"><div className="sectionHeading splitHeading"><div><span className="kicker">Kategori & koleksi</span><h2 id="featured-products-title">Bunga pilihan untuk momen yang berarti.</h2></div><p>Jelajahi kategori favorit, lalu pilih dari koleksi unggulan Biorona. Semua harga tetap jelas sebelum Anda lanjut ke WhatsApp.</p></div><div className="homeCategories" aria-label="Kategori bunga">{categoryOptions.filter((item) => item !== "Semua").map((item) => <a key={item} href={`/katalog?category=${encodeURIComponent(item)}`}>{item}</a>)}</div>{unavailableMessage ? <div className="emptyState" role="alert"><strong>{unavailableMessage}</strong><p>Anda tetap dapat menghubungi Biorona melalui WhatsApp untuk bantuan.</p></div> : <><div className="productGrid">{featured.map((product) => <ProductCard key={product.slug} product={product}/>)}</div><div className="catalogAllCta"><a className="secondaryGlassButton glassSurface" href="/katalog">Lihat Semua Koleksi <span aria-hidden="true">→</span></a></div></>}</div></section>;
