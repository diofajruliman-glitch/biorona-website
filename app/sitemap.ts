...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    return [root, ...products.filter(isSearchIndexableProduct).map((product) => ({ url: `${siteConfig.siteUrl}/produk/${product.slug}/`, changeFrequency: "weekly" as const, priority: .8 }))];
  } catch (error) {
