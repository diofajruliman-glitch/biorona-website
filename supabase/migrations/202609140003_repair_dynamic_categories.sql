...
set search_path = ''
as $$
  select lower(regexp_replace(regexp_replace(trim(value), '[^a-zA-Z0-9]+', '-', 'g'), '^-+|-+$', '', 'g'));
$$;
...
update public.categories
set slug = concat(coalesce(nullif(public.slugify_category_name(name), ''), 'kategori'), '-', substr(id::text, 1, 8))
where slug is null
...
select legacy.name,
       concat(coalesce(nullif(public.slugify_category_name(legacy.name), ''), 'kategori'), '-', substr(md5(lower(legacy.name)), 1, 8)),
       null,
       0,
       true
from (
...
where not exists (
  select 1 from public.categories c where lower(btrim(c.name)) = lower(legacy.name)
);
...
where exists (
  select 1 from public.products where category_id is null and (category is null or length(btrim(category)) = 0)
)
and not exists (
  select 1 from public.categories where lower(btrim(name)) = 'tanpa kategori'
);
...
begin
  if exists (select 1 from public.products where category_id is null) then
    raise exception 'Repair dihentikan: masih ada produk tanpa category_id yang dapat dipetakan.';
  end if;
end $$;
...
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'categories_name_not_blank' and conrelid = 'public.categories'::regclass) then
    alter table public.categories add constraint categories_name_not_blank check (length(btrim(name)) > 0);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'categories_slug_not_blank' and conrelid = 'public.categories'::regclass) then
    alter table public.categories add constraint categories_slug_not_blank check (length(btrim(slug)) > 0);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'categories_slug_format' and conrelid = 'public.categories'::regclass) then
...
begin
  if not exists (
    ...
    create unique index if not exists categories_name_unique_ci on public.categories (lower(btrim(name)));
  end if;
end $$;
...
begin
  if exists (
    ...
    raise exception using errcode = '23505', message = 'Nama kategori sudah digunakan.';
  end if;
  return new;
end;
$$;

drop trigger if exists categories_prevent_duplicate_name on public.categories;
...
begin
  if exists (select 1 from public.products where category_id = old.id) then
    raise exception using errcode = '23503', message = 'Kategori ini masih digunakan oleh produk.';
  end if;
  return old;
end;
$$;
...
begin
  ...
  if new.category is null then
    raise exception using errcode = '23503', message = 'Kategori produk tidak valid atau sudah tidak tersedia.';
  end if;
  return new;
end;
$$;

-- TODO: drop products.category only after every legacy consumer has been retired.
